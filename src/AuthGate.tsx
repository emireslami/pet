...
export default function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  return <main className="auth-page" dir="rtl">
    <section className="auth-brand"><div className="brand-symbol">♥</div><span>پت‌پرونده</span><h1>پرونده سلامت پت‌ها، برای تمام اعضای خانواده</h1><p>یک پت می‌تواند چند سرپرست داشته باشد و هر سرپرست با شماره موبایل خودش به پرونده‌های مجاز دسترسی دارد.</p><div className="family-access"><span>پدر</span><i>+</i><span>مادر</span><i>+</i><span>دامپزشک</span><b>یک پرونده مشترک</b></div></section>
    <section className="auth-card"><div><SafetyCertificateOutlined /><Title level={2}>{mode === "login" ? "ورود به حساب" : "ساخت حساب جدید"}</Title><Text type="secondary">با شماره موبایل و رمز ثابت وارد شوید</Text></div>
      ...
      <Form layout="vertical" onFinish={submit} requiredMark={false}>
        ...
        <Button htmlType="submit" type="primary" size="large" block loading={busy}>{mode === "login" ? "ورود به پت‌پرونده" : "ساخت حساب"}</Button>
      </Form>
