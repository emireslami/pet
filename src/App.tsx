...
function Dashboard({pets,timelineRows,onAddPet,onAddRecord,onOpen}:{pets:Pet[];timelineRows:MedicalTimelineRow[];onAddPet:()=>void;onAddRecord:()=>void;onOpen:(p:Pet)=>void}) {
  ...
  <SectionHead title="آخرین فعالیت‌ها"/>{!timelineRows.length?<EmptyPanel title="هنوز رویدادی ثبت نشده" text="ویزیت، نسخه، آزمایش یا هر مدرک پزشکی را ثبت کنید." action={pets.length?<Button onClick={onAddRecord}>ثبت اولین رویداد</Button>:undefined}/>:<MedicalTimelineTable rows={timelineRows.slice(0,8)} pets={pets}/>}</>;
}
...
export default function App(){
  ...
  const content=()=>{if(loading)return <div className="page-loading"><CircularProgress/></div>;if(pet)return <PetProfile pet={pet} records={records} care={care} surgeries={surgeries} certificates={certificates} timelineRows={timelineRows} back={()=>setPet(null)} onAdd={()=>setRecordModal(true)} onRefresh={load} onPetUpdated={updateSelectedPet}/>;if(page==="account")return <UserProfilePage/>;if(page==="dashboard"||page==="pets")return <Dashboard pets={pets} timelineRows={timelineRows} onAddPet={()=>setPetModal(true)} onAddRecord={()=>setRecordModal(true)} onOpen={setPet}/>;if(page==="preventive")return <GlobalPreventivePage pets={pets} care={care} surgeries={surgeries} certificates={certificates} weights={weights} onOpen={setPet}/>;if(page==="timeline")return <><SectionHead title="تایم‌لاین پزشکی" action={<Button variant="contained" disabled={!pets.length} onClick={()=>setRecordModal(true)}>ثبت رویداد</Button>}/>{timelineRows.length?<MedicalTimelineTable rows={timelineRows} pets={pets}/>:<EmptyPanel title="تایم‌لاین خالی است" text="هیچ داده پزشکی، واکسن، وزن یا میکروچیپی ثبت نشده است."/ >}</>;return <><SectionHead title={pageTitle}/><EmptyPanel title="هنوز اطلاعاتی ثبت نشده" text="این بخش با ثبت اطلاعات واقعی شما تکمیل می‌شود."/ ></>;};
