...
function JalaliDateField({label,value,onChange,optional=true,helperText}:{label:string;value?:string|null;onChange:(next:{jalali:string;gregorian:string})=>void;optional?:boolean;helperText?:string}) {
  ...
  return <Box className="pet-birth-card subtle"><Typography variant="subtitle2">{label}</Typography>{helperText&&<Typography color="text.secondary" variant="caption">{helperText}</Typography>}<div className="form-row birth-row"><FormControl><InputLabel>سال</InputLabel><Select label="سال" value={parts.year} onChange={e=>update("year",String(e.target.value))}>{optional&&<MenuItem value="">ثبت نشده</MenuItem>}{jalaliYears.map(y=><MenuItem key={y} value={String(y)}>{y}</MenuItem>)}</Select></FormControl><FormControl><InputLabel>ماه</InputLabel><Select label="ماه" value={parts.month} onChange={e=>update("month",String(e.target.value))}><MenuItem value="">فروردین پیش‌فرض</MenuItem>{persianMonths.map((m,i)=><MenuItem key={m} value={String(i+1)}>{m}</MenuItem>)}</Select></FormControl><FormControl><InputLabel>روز</InputLabel><Select label="روز" value={parts.day} onChange={e=>update("day",String(e.target.value))}><MenuItem value="">روز ۱ پیش‌فرض</MenuItem>{Array.from({length:jalaliMonthDays(parts.year,parts.month)},(_,i)=>i+1).map(day=><MenuItem key={day} value={String(day)}>{day}</MenuItem>)}</Select></FormControl></div>{value&&<Typography color="text.secondary" variant="caption">ذخیره شمسی: {value}</Typography>}</Box>;
}
...
function PetProfile({pet,records,care,surgeries,certificates,back,onAdd,onRefresh,onPetUpdated}:{pet:Pet;records:MedicalRecord[];care:PreventiveCareRecord[];surgeries:SurgicalHistoryRecord[];certificates:RabiesAntibodyCertificate[];back:()=>void;onAdd:()=>void;onRefresh:()=>void;onPetUpdated:(pet:Pet)=>void}) {
  ...
  const[share,setShare]=useState(false),[tab,setTab]=useState(0),[draft,setDraft]=useState(petDraft(pet)),[saving,setSaving]=useState(false);const list=records.filter(r=>r.pet_id===pet.id);
