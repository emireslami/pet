...
export default function App(){
  ...
  const content=()=>{if(loading)return <div className="page-loading"><CircularProgress/></div>;if(pet)return <PetProfile pet={pet} records={records} care={care} surgeries={surgeries} certificates={certificates} back={()=>setPet(null)} onAdd={()=>setRecordModal(true)} onRefresh={load} onPetUpdated={updateSelectedPet}/>;if(page==="account")return <UserProfilePage/>;if(page==="dashboard"||page==="pets")return <Dashboard pets={pets} records={records} onAddPet={()=>setPetModal(true)} onAddRecord={()=>setRecordModal(true)} onOpen={setPet}/>;if(page==="preventive")return <GlobalPreventivePage pets={pets} care={care} surgeries={surgeries} certificates={certificates} weights={weights} onOpen={setPet}/>;if(page==="timeline")return <><SectionHead title="تایم‌لاین پزشکی" action={<Button variant="contained" disabled={!pets.length} onClick={()=>setRecordModal(true)}>ثبت رویداد</Button>}/>{records.length?<RecordTable records={records} pets={pets}/>:<EmptyPanel title="تایم‌لاین خالی است" text="هیچ رویداد پزشکی ثبت نشده است."/ >}</>;return <><SectionHead title={pageTitle}/><EmptyPanel title="هنوز اطلاعاتی ثبت نشده" text="این بخش با ثبت اطلاعات واقعی شما تکمیل می‌شود."/ ></>;};
