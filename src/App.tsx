...
import {
  AddRounded, AppsRounded, ArchiveRounded, CalendarMonthRounded, CheckCircleRounded,
...
function PetProfile({pet,records,care,surgeries,certificates,timelineRows,back,onAdd,onRefresh,onPetUpdated,onArchive}:{pet:Pet;records:MedicalRecord[];care:PreventiveCareRecord[];surgeries:SurgicalHistoryRecord[];certificates:RabiesAntibodyCertificate[];timelineRows:MedicalTimelineRow[];back:()=>void;onAdd:()=>void;onRefresh:()=>void;onPetUpdated:(pet:Pet)=>void;onArchive:(pet:Pet)=>void}) {
...
export default function App(){
  ...
  const load=useCallback(async()=>{if(!hasSupabase){setLoading(false);return;}setLoading(true);const[p,r,c,s,cert,w]=await Promise.all([supabase.from("pets").select("*").is("archived_at",null).order("created_at"),supabase.from("medical_records").select("*").order("created_at",{ascending:false}),supabase.from("pet_preventive_care_records").select("*").order("created_at",{ascending:false}),supabase.from("pet_surgical_history").select("*").order("created_at",{ascending:false}),supabase.from("pet_rabies_antibody_certificates").select("*").order("created_at",{ascending:false}),supabase.from("pet_weight_records").select("*").order("created_at",{ascending:false})]);const raw=(p.data||[])as Pet[];const withPhotos=await Promise.all(raw.map(async pet=>{const listed=await supabase.storage.from("pet-documents").list(pet.id,{search:"avatar",limit:1});if(!listed.data?.length)return pet;const signed=await supabase.storage.from("pet-documents").createSignedUrl(`${pet.id}/avatar`,3600);return{...pet,photo_url:signed.data?.signedUrl||null};}));setPets(withPhotos);setRecords((r.data||[])as MedicalRecord[]);setCare((c.data||[])as PreventiveCareRecord[]);setSurgeries((s.data||[])as SurgicalHistoryRecord[]);setCertificates((cert.data||[])as RabiesAntibodyCertificate[]);setWeights((w.data||[])as PetWeightRecord[]);setLoading(false);},[]);
