...
import type { MedicalRecord, Pet, PreventiveCareRecord, RabiesAntibodyCertificate, SurgicalHistoryRecord, UserProfile } from "./types";
...
function PetForm({open,onClose,onSaved}:{open:boolean;onClose:()=>void;onSaved:()=>void}) {
  const [values,setValues]=useState({name:"",species:"",species_id:"",breed:"",breed_id:"",gender:"",birth_date:"",current_weight:"",color:""});
  ...
  const resetForm=()=>{setValues({name:"",species:"",species_id:"",breed:"",breed_id:"",gender:"",birth_date:"",current_weight:"",color:""});setPhoto(null);setPhotoPreview("");setCustomSpecies("");setCustomBreed("");setBirth({year:"",month:"",day:""});setBreedOptions([]);};
...
type CareDraft={id?:string;type:PreventiveCareRecord["type"];substance_name:string;administered_at:string;next_relevant_date:string;veterinarian_name:string;notes:string};
const emptyCareDraft=(type:PreventiveCareRecord["type"]="RABIES_VACCINATION"):CareDraft=>({type,substance_name:"",administered_at:"",next_relevant_date:"",veterinarian_name:"",notes:""});
...
function UserProfilePage(){
  ...
  return <><SectionHead title="حساب کاربری" action={<Button variant="contained" onClick={save} disabled={busy}>{busy?<CircularProgress size={18}/>:"ذخیره"}</Button>}/>{message&&<Alert severity={message.includes("نشد")?"error":"success"} sx={{mb:2}}>{message}</Alert>}<div className="profile-edit-grid"><Card><CardContent><Typography variant="h6">اطلاعات فارسی</Typography><Stack spacing={2} sx={{mt:2}}><div className="form-row"><TextField label="نام فارسی" value={profile.first_name_fa||""} onChange={setField("first_name_fa")}/><TextField label="نام خانوادگی فارسی" value={profile.last_name_fa||""} onChange={setField("last_name_fa")}/></div><TextField label="آدرس فارسی" value={profile.address_fa||""} onChange={setField("address_fa")} multiline minRows={3}/><TextField label="شماره موبایل" value={profile.phone_number||profile.phone||""} onChange={setField("phone_number")} slotProps={{htmlInput:{inputMode:"numeric"}}}/></Stack></CardContent></Card><Card><CardContent><Typography variant="h6">English Information</Typography><Stack spacing={2} sx={{mt:2}}><div className="form-row"><TextField label="First name" value={profile.first_name_en||""} onChange={setField("first_name_en")}/><TextField label="Last name" value={profile.last_name_en||""} onChange={setField("last_name_en")}/></div><TextField label="Address" value={profile.address_en||""} onChange={setField("address_en")} multiline minRows={3}/></Stack></CardContent></Card></div></>;
}
...
function PreventiveCarePanel({pet,care,surgeries,certificates,onRefresh}:{pet:Pet;care:PreventiveCareRecord[];surgeries:SurgicalHistoryRecord[];certificates:RabiesAntibodyCertificate[];onRefresh:()=>void}) {
  const[careDraft,setCareDraft]=useState<CareDraft>(emptyCareDraft()),[surgery,setSurgery]=useState({id:"",procedure_name:"",description:"",performed_at:"",veterinarian_name:""}),[cert,setCert]=useState({id:"",veterinarian_name:"",address:"",telephone_number:"",email:"",issued_at:""}),[busy,setBusy]=useState("");
  ...
  const saveCare=async()=>{setBusy("care");const payload={pet_id:pet.id,type:careDraft.type,substance_name:careDraft.substance_name||null,administered_at:careDraft.administered_at||null,next_relevant_date:careDraft.next_relevant_date||null,veterinarian_name:careDraft.veterinarian_name||null,notes:careDraft.notes||null,updated_at:new Date().toISOString()};const query=careDraft.id?supabase.from("pet_preventive_care_records").update(payload).eq("id",careDraft.id):supabase.from("pet_preventive_care_records").insert(payload);await query;setBusy("");setCareDraft(emptyCareDraft(careDraft.type));onRefresh();};
  const saveSurgery=async()=>{setBusy("surgery");const payload={pet_id:pet.id,procedure_name:surgery.procedure_name||null,description:surgery.description||null,performed_at:surgery.performed_at||null,veterinarian_name:surgery.veterinarian_name||null,updated_at:new Date().toISOString()};const query=surgery.id?supabase.from("pet_surgical_history").update(payload).eq("id",surgery.id):supabase.from("pet_surgical_history").insert(payload);await query;setBusy("");setSurgery({id:"",procedure_name:"",description:"",performed_at:"",veterinarian_name:""});onRefresh();};
  const saveCert=async()=>{setBusy("cert");const payload={pet_id:pet.id,veterinarian_name:cert.veterinarian_name||null,address:cert.address||null,telephone_number:numericOnly(cert.telephone_number)||null,email:cert.email||null,issued_at:cert.issued_at||null,updated_at:new Date().toISOString()};const query=cert.id?supabase.from("pet_rabies_antibody_certificates").update(payload).eq("id",cert.id):supabase.from("pet_rabies_antibody_certificates").insert(payload);await query;setBusy("");setCert({id:"",veterinarian_name:"",address:"",telephone_number:"",email:"",issued_at:""});onRefresh();};
  ...
  return <Stack spacing={2}><Card><CardContent><Typography variant="h6">مراقبت پیشگیرانه</Typography><Stack spacing={2} sx={{mt:2}}><div className="form-row"><FormControl><InputLabel>دسته‌بندی</InputLabel><Select label="دسته‌بندی" value={careDraft.type} onChange={e=>setCareDraft(emptyCareDraft(e.target.value as PreventiveCareRecord["type"]))}>{careTypes.map(type=><MenuItem key={type} value={type}>{careLabels[type].title}</MenuItem>)}</Select></FormControl><TextField label={currentLabel.substance} value={careDraft.substance_name} onChange={e=>setCareDraft(v=>({...v,substance_name:e.target.value}))}/></div><div className="form-row"><TextField label={currentLabel.date} type="date" value={careDraft.administered_at} onChange={e=>setCareDraft(v=>({...v,administered_at:e.target.value}))} slotProps={{inputLabel:{shrink:true}}}/><TextField label={currentLabel.next} type="date" value={careDraft.next_relevant_date} onChange={e=>setCareDraft(v=>({...v,next_relevant_date:e.target.value}))} slotProps={{inputLabel:{shrink:true}}}/></div><TextField label="نام دامپزشک" value={careDraft.veterinarian_name} onChange={e=>setCareDraft(v=>({...v,veterinarian_name:e.target.value}))}/><TextField label="یادداشت" value={careDraft.notes} onChange={e=>setCareDraft(v=>({...v,notes:e.target.value}))} multiline minRows={2}/><Button variant="contained" onClick={saveCare} disabled={busy==="care"}>{careDraft.id?"ذخیره ویرایش":"افزودن رکورد"}</Button></Stack><div className="record-list">{petCare.map(item=><Paper key={item.id} variant="outlined" className="record-item"><div><b>{careLabels[item.type].title}</b><small>{[item.substance_name,item.administered_at,item.next_relevant_date,item.veterinarian_name].filter(Boolean).join(" · ")}</small></div><Stack direction="row"><Button onClick={()=>setCareDraft({id:item.id,type:item.type,substance_name:item.substance_name||"",administered_at:item.administered_at||"",next_relevant_date:item.next_relevant_date||"",veterinarian_name:item.veterinarian_name||"",notes:item.notes||""})}>ویرایش</Button><IconButton onClick={()=>remove("pet_preventive_care_records",item.id)}><DeleteRounded/></IconButton></Stack></Paper>)}</div></CardContent></Card><Card><CardContent><Typography variant="h6">تاریخچه جراحی</Typography><Stack spacing={2} sx={{mt:2}}><div className="form-row"><TextField label="جراحی / پروسیجر" value={surgery.procedure_name} onChange={e=>setSurgery(v=>({...v,procedure_name:e.target.value}))}/><TextField label="تاریخ جراحی" type="date" value={surgery.performed_at} onChange={e=>setSurgery(v=>({...v,performed_at:e.target.value}))} slotProps={{inputLabel:{shrink:true}}}/></div><TextField label="نام دامپزشک" value={surgery.veterinarian_name} onChange={e=>setSurgery(v=>({...v,veterinarian_name:e.target.value}))}/><TextField label="توضیحات" value={surgery.description} onChange={e=>setSurgery(v=>({...v,description:e.target.value}))} multiline minRows={2}/><Button variant="contained" onClick={saveSurgery} disabled={busy==="surgery"}>{surgery.id?"ذخیره ویرایش":"افزودن جراحی"}</Button></Stack><div className="record-list">{petSurgeries.map(item=><Paper key={item.id} variant="outlined" className="record-item"><div><b>{item.procedure_name||"جراحی"}</b><small>{[item.performed_at,item.veterinarian_name].filter(Boolean).join(" · ")}</small></div><Stack direction="row"><Button onClick={()=>setSurgery({id:item.id,procedure_name:item.procedure_name||"",description:item.description||"",performed_at:item.performed_at||"",veterinarian_name:item.veterinarian_name||""})}>ویرایش</Button><IconButton onClick={()=>remove("pet_surgical_history",item.id)}><DeleteRounded/></IconButton></Stack></Paper>)}</div></CardContent></Card><Card><CardContent><Typography variant="h6">Rabies Antibody Titre</Typography><Stack spacing={2} sx={{mt:2}}><div className="form-row"><TextField label="نام دامپزشک مجاز" value={cert.veterinarian_name} onChange={e=>setCert(v=>({...v,veterinarian_name:e.target.value}))}/><TextField label="تاریخ" type="date" value={cert.issued_at} onChange={e=>setCert(v=>({...v,issued_at:e.target.value}))} slotProps={{inputLabel:{shrink:true}}}/></div><div className="form-row"><TextField label="تلفن" value={cert.telephone_number} onChange={e=>setCert(v=>({...v,telephone_number:numericOnly(e.target.value)}))}/><TextField label="ایمیل" value={cert.email} onChange={e=>setCert(v=>({...v,email:e.target.value}))}/></div><TextField label="آدرس" value={cert.address} onChange={e=>setCert(v=>({...v,address:e.target.value}))} multiline minRows={2}/><Button variant="contained" onClick={saveCert} disabled={busy==="cert"}>{cert.id?"ذخیره ویرایش":"افزودن گواهی"}</Button></Stack><div className="record-list">{petCerts.map(item=><Paper key={item.id} variant="outlined" className="record-item"><div><b>{item.veterinarian_name||"گواهی هاری"}</b><small>{[item.issued_at,item.telephone_number,item.email].filter(Boolean).join(" · ")}</small></div><Stack direction="row"><Button onClick={()=>setCert({id:item.id,veterinarian_name:item.veterinarian_name||"",address:item.address||"",telephone_number:item.telephone_number||"",email:item.email||"",issued_at:item.issued_at||""})}>ویرایش</Button><IconButton onClick={()=>remove("pet_rabies_antibody_certificates",item.id)}><DeleteRounded/></IconButton></Stack></Paper>)}</div></CardContent></Card></Stack>;
}

function PetProfile({pet,records,care,surgeries,certificates,back,onAdd,onRefresh,onPetUpdated}:{pet:Pet;records:MedicalRecord[];care:PreventiveCareRecord[];surgeries:SurgicalHistoryRecord[];certificates:RabiesAntibodyCertificate[];back:()=>void;onAdd:()=>void;onRefresh:()=>void;onPetUpdated:(pet:Pet)=>void}) {
  const[share,setShare]=useState(false),[tab,setTab]=useState(0),[draft,setDraft]=useState({...pet,current_weight:pet.current_weight?String(pet.current_weight):"",is_neutered:pet.is_neutered==null?"":pet.is_neutered?"true":"false"}),[saving,setSaving]=useState(false);const list=records.filter(r=>r.pet_id===pet.id);
  useEffect(()=>setDraft({...pet,current_weight:pet.current_weight?String(pet.current_weight):"",is_neutered:pet.is_neutered==null?"":pet.is_neutered?"true":"false"}),[pet]);
  const setDraftField=(key:string)=>(e:{target:{value:unknown}})=>setDraft(v=>({...v,[key]:String(e.target.value)}));
  const savePet=async()=>{setSaving(true);const payload={name:draft.name,species:draft.species,breed:draft.breed||null,gender:draft.gender||null,birth_date:draft.birth_date||null,current_weight:draft.current_weight?Number(decimalOnly(String(draft.current_weight))):null,color:draft.color||null,distinctive_marks:draft.distinctive_marks||null,is_neutered:draft.is_neutered===""?null:draft.is_neutered==="true",passport_number:draft.passport_number||null,passport_issue_date:draft.passport_issue_date||null,microchip_number:numericOnly(draft.microchip_number||"")||null,microchip_implant_date:draft.microchip_implant_date||null,issuing_veterinarian_name:draft.issuing_veterinarian_name||null,updated_at:new Date().toISOString()};const{data,error}=await supabase.from("pets").update(payload).eq("id",pet.id).select("*").single();setSaving(false);if(!error&&data){onPetUpdated({...data,photo_url:pet.photo_url} as Pet);onRefresh();}};
  return <><div className="profile-actions"><Button onClick={back}>→ بازگشت</Button><Stack direction="row"><Button startIcon={<GroupsRounded/>} onClick={()=>setShare(true)}>مدیریت دسترسی</Button><Button variant="contained" startIcon={<AddRounded/>} onClick={onAdd}>ثبت اطلاعات</Button></Stack></div><Card className="profile-head"><CardContent><div className="profile-main"><PetMark pet={pet} size={82}/><div><Typography variant="h4">{pet.name}</Typography><Typography color="text.secondary">{[pet.species,pet.breed,pet.gender].filter(Boolean).join(" · ")}</Typography></div></div><div className="pet-card-data">{[["تاریخ تولد",formatJalaliDate(pet.birth_date)||"ثبت نشده"],["وزن",pet.current_weight?`${pet.current_weight} کیلوگرم`:"ثبت نشده"],["رنگ",emptyText(pet.color)],["میکروچیپ",pet.microchip_number||"ثبت نشده"],["تعداد سوابق",list.length]].map(([a,b])=><div key={String(a)}><small>{a}</small><b>{b}</b></div>)}</div></CardContent></Card><Tabs value={tab} onChange={(_,v)=>setTab(v)}><Tab label="پروفایل"/><Tab label="پاسپورت"/><Tab label="مراقبت پیشگیرانه"/><Tab label="اسناد"/></Tabs><Box sx={{mt:2}}>{tab===0&&<Card><CardContent><SectionHead title="پروفایل پت" action={<Button variant="contained" onClick={savePet} disabled={saving}>{saving?<CircularProgress size={18}/>:"ذخیره"}</Button>}/><Stack spacing={2}><div className="form-row"><TextField label="نام" value={draft.name||""} onChange={setDraftField("name")}/><TextField label="گونه" value={draft.species||""} onChange={setDraftField("species")}/></div><div className="form-row"><TextField label="نژاد" value={draft.breed||""} onChange={setDraftField("breed")}/><FormControl><InputLabel>جنسیت</InputLabel><Select label="جنسیت" value={draft.gender||""} onChange={setDraftField("gender")}><MenuItem value="">ثبت نشده</MenuItem><MenuItem value="نر">نر</MenuItem><MenuItem value="ماده">ماده</MenuItem></Select></FormControl></div><div className="form-row"><TextField label="تاریخ تولد" type="date" value={draft.birth_date||""} onChange={setDraftField("birth_date")} slotProps={{inputLabel:{shrink:true}}}/><TextField label="وزن" value={String(draft.current_weight||"")} onChange={e=>setDraft(v=>({...v,current_weight:decimalOnly(e.target.value)}))} slotProps={{input:{endAdornment:<InputAdornment position="end">کیلوگرم</InputAdornment>}}}/></div><div className="form-row"><TextField label="رنگ" value={draft.color||""} onChange={setDraftField("color")}/><FormControl><InputLabel>عقیم شده؟</InputLabel><Select label="عقیم شده؟" value={draft.is_neutered} onChange={setDraftField("is_neutered")}><MenuItem value="">ثبت نشده</MenuItem><MenuItem value="true">بله</MenuItem><MenuItem value="false">خیر</MenuItem></Select></FormControl></div><TextField label="علائم متمایز" value={draft.distinctive_marks||""} onChange={setDraftField("distinctive_marks")} multiline minRows={2}/></Stack></CardContent></Card>}{tab===1&&<Card><CardContent><SectionHead title="اطلاعات پاسپورت و شناسایی" action={<Button variant="contained" onClick={savePet} disabled={saving}>{saving?<CircularProgress size={18}/>:"ذخیره"}</Button>}/><Stack spacing={2}><div className="form-row"><TextField label="شماره پاسپورت / گواهی" value={draft.passport_number||""} onChange={setDraftField("passport_number")}/><TextField label="تاریخ صدور" type="date" value={draft.passport_issue_date||""} onChange={setDraftField("passport_issue_date")} slotProps={{inputLabel:{shrink:true}}}/></div><div className="form-row"><TextField label="شماره میکروچیپ" value={draft.microchip_number||""} onChange={e=>setDraft(v=>({...v,microchip_number:numericOnly(e.target.value)}))}/><TextField label="تاریخ کاشت میکروچیپ" type="date" value={draft.microchip_implant_date||""} onChange={setDraftField("microchip_implant_date")} slotProps={{inputLabel:{shrink:true}}}/></div><TextField label="نام دامپزشک صادرکننده" value={draft.issuing_veterinarian_name||""} onChange={setDraftField("issuing_veterinarian_name")}/></Stack></CardContent></Card>}{tab===2&&<PreventiveCarePanel pet={pet} care={care} surgeries={surgeries} certificates={certificates} onRefresh={onRefresh}/>} {tab===3&&(list.length?<RecordTable records={list} pets={[pet]}/>:<EmptyPanel title="سندی ثبت نشده" text="اسناد پیوست‌شده اینجا نمایش داده می‌شوند." action={<Button onClick={onAdd}>ثبت اطلاعات</Button>}/>)}</Box><SharePetModal pet={pet} open={share} onClose={()=>{setShare(false);onRefresh();}}/></>;
}

function GlobalPreventivePage({pets,care,surgeries,certificates,onOpen}:{pets:Pet[];care:PreventiveCareRecord[];surgeries:SurgicalHistoryRecord[];certificates:RabiesAntibodyCertificate[];onOpen:(pet:Pet)=>void}) {
  const petName=(id:string)=>pets.find(p=>p.id===id)?.name||"پت";
  const petFor=(id:string)=>pets.find(p=>p.id===id);
  const rows=[
    ...care.map(item=>({id:item.id,pet_id:item.pet_id,title:careLabels[item.type].title,body:[item.substance_name,item.administered_at,item.next_relevant_date,item.veterinarian_name].filter(Boolean).join(" · "),date:item.administered_at||item.created_at||""})),
    ...surgeries.map(item=>({id:item.id,pet_id:item.pet_id,title:"تاریخچه جراحی",body:[item.procedure_name,item.performed_at,item.veterinarian_name].filter(Boolean).join(" · "),date:item.performed_at||item.created_at||""})),
    ...certificates.map(item=>({id:item.id,pet_id:item.pet_id,title:"Rabies Antibody Titre",body:[item.veterinarian_name,item.issued_at,item.telephone_number].filter(Boolean).join(" · "),date:item.issued_at||item.created_at||""})),
...
export default function App(){
  ...
  const load=useCallback(async()=>{if(!hasSupabase){setLoading(false);return;}setLoading(true);const[p,r,c,s,cert]=await Promise.all([supabase.from("pets").select("*").order("created_at"),supabase.from("medical_records").select("*").order("created_at",{ascending:false}),supabase.from("pet_preventive_care_records").select("*").order("created_at",{ascending:false}),supabase.from("pet_surgical_history").select("*").order("created_at",{ascending:false}),supabase.from("pet_rabies_antibody_certificates").select("*").order("created_at",{ascending:false})]);const raw=(p.data||[])as Pet[];const withPhotos=await Promise.all(raw.map(async pet=>{const listed=await supabase.storage.from("pet-documents").list(pet.id,{search:"avatar",limit:1});if(!listed.data?.length)return pet;const signed=await supabase.storage.from("pet-documents").createSignedUrl(`${pet.id}/avatar`,3600);return{...pet,photo_url:signed.data?.signedUrl||null};}));setPets(withPhotos);setRecords((r.data||[])as MedicalRecord[]);setCare((c.data||[])as PreventiveCareRecord[]);setSurgeries((s.data||[])as SurgicalHistoryRecord[]);setCertificates((cert.data||[])as RabiesAntibodyCertificate[]);setLoading(false);},[]);
  ...
  const content=()=>{if(loading)return <div className="page-loading"><CircularProgress/></div>;if(pet)return <PetProfile pet={pet} records={records} care={care} surgeries={surgeries} certificates={certificates} back={()=>setPet(null)} onAdd={()=>setRecordModal(true)} onRefresh={load} onPetUpdated={updateSelectedPet}/>;if(page==="account")return <UserProfilePage/>;if(page==="dashboard"||page==="pets")return <Dashboard pets={pets} records={records} onAddPet={()=>setPetModal(true)} onAddRecord={()=>setRecordModal(true)} onOpen={setPet}/>;if(page==="preventive")return <GlobalPreventivePage pets={pets} care={care} surgeries={surgeries} certificates={certificates} onOpen={setPet}/>;if(page==="timeline")return <><SectionHead title="تایم‌لاین پزشکی" action={<Button variant="contained" disabled={!pets.length} onClick={()=>setRecordModal(true)}>ثبت رویداد</Button>}/>{records.length?<RecordTable records={records} pets={pets}/>:<EmptyPanel title="تایم‌لاین خالی است" text="هیچ رویداد پزشکی ثبت نشده است."/ >}</>;return <><SectionHead title={pageTitle}/><EmptyPanel title="هنوز اطلاعاتی ثبت نشده" text="این بخش با ثبت اطلاعات واقعی شما تکمیل می‌شود."/ ></>;};
